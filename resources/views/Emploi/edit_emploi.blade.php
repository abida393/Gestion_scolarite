...
<div class="container mx-auto p-6">
    <h1 class="text-3xl font-bold mb-6">Modifier un Emploi du Temps</h1>

    <form action="{{ route('emploi.edite_emploi', $emploiTemp) }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <input type="time" name="heure_debut" id="heure_debut" value="{{ $emploiTemp->heure_debut }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
        </div>

        <div class="mb-4">
            <label for="heure_fin" class="block text-sm font-medium text-gray-700">Heure de fin</label>
            <input type="time" name="heure_fin" id="heure_fin" value="{{ $emploiTemp->heure_fin }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="matiere_id" id="matiere_id" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
                @foreach ($matieres as $matiere)
                    <option value="{{ $matiere->id }}" {{ $emploiTemp->matiere_id == $matiere->id ? 'selected' : '' }}>{{ $matiere->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="enseignant_id" id="enseignant_id" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
                @foreach ($enseignants as $enseignant)
                    <option value="{{ $enseignant->id }}" {{ $emploiTemp->enseignant_id == $enseignant->id ? 'selected' : '' }}>{{ $enseignant->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="classe_id" id="classe_id" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500" required>
                @foreach ($classes as $classe)
                    <option value="{{ $classe->id }}" {{ $emploiTemp->classe_id == $classe->id ? 'selected' : '' }}>{{ $classe->name }}</option>
                @endforeach
            </select>
        </div>

        <div class="flex justify-end">
            <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded-lg">Mettre à jour</button>
        </div>
    </form>
</div>
