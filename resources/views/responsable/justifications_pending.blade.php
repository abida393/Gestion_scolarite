                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse($justifications as $justif)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                ...
                                                <span
                                                    class="text-blue-600 font-medium">{{ substr($justif->etudiant->etudiant_nom, 0, 1) }}{{ substr($justif->etudiant->etudiant_prenom, 0, 1) }}</span>
                                            </div>
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">
                                                    {{ $justif->etudiant->etudiant_nom }}
                                                    {{ $justif->etudiant->etudiant_prenom }}</div>
                                                <div class="text-sm text-gray-500">
                                                    {{ $justif->etudiant->etudiant_email ?? '' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <div class="text-sm text-gray-500">
                                            @if ($justif->emploiTemps && $justif->emploiTemps->heure_debut && $justif->emploiTemps->heure_fin)
                                                {{ \Carbon\Carbon::parse($justif->emploiTemps->heure_debut)->format('H:i') }}
                                                -
                                                {{ \Carbon\Carbon::parse($justif->emploiTemps->heure_fin)->format('H:i') }}
                                            @else
                                                Horaire inconnu
                                            @endif
                                        </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <form action="{{ route('responsable.absences.validate', $justif->id) }}"
                                            method="POST">
                                            @csrf
                                            <div class="flex space-x-2">
                                                <select name="action"
                                                    ...
                                                    <option value="non_justifier" class="text-yellow-600">Non justifier
                                                    </option>
                                                </select>
                                                <button type="submit"
                                ...
                                <tr>
                                    <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">
                                        <div class="flex flex-col items-center justify-center py-8">
                                            ...
                                            <h3 class="mt-2 text-sm font-medium text-gray-900">Aucune justification en
                                                attente</h3>
                                            <p class="mt-1 text-sm text-gray-500">Toutes les justifications ont été
                                                traitées.</p>
                                        </div>
                                    </td>
                                </tr>
                ...
                @if ($justifications->hasPages())
                    <div class="mt-6 flex items-center justify-between">
                        <div class="flex-1 flex justify-between sm:hidden">
                            ...
                            @else
                                <a href="{{ $justifications->previousPageUrl() }}"
                            ...
                            @if ($justifications->hasMorePages())
                                <a href="{{ $justifications->nextPageUrl() }}"
