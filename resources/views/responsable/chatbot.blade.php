<x-admin titre="chatbot" page_titre="chatbot">
<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    ...
    <div class="max-w-3xl mx-auto bg-white p-6 rounded-2xl shadow-lg">
        ...
        <form method="POST" action="{{ route('chatbot.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="mot_cle" id="mot_cle" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
            </div>
            <div>
                <label for="reponse" class="block text-gray-700 font-semibold">Réponse</label>
                <textarea name="reponse" id="reponse" rows="3" class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required></textarea>
            </div>
            ...
            @foreach($reponses as $rep)
                <div class="bg-gray-50 border rounded p-4 space-y-2">
                    <div class="flex flex-col md:flex-row md:items-center justify-between">
                        ...
                        <button onclick="toggleEditForm({{ $rep->id }})" class="bg-green-500 text-white px-3 py-1 rounded hover:bg-green-600 transition">Modifier</button>
                    ...
                    <form action="{{ route('chatbot.update', $rep->id) }}" method="POST" id="edit-form-{{ $rep->id }}" class="edit-form hidden space-y-3 mt-3">
                        @csrf
                        @method('PUT')
                        <div>
                            <label class="text-sm font-medium text-gray-700">Mot Clé</label>
                            <input type="text" name="mot_cle" value="{{ $rep->mot_cle }}" class="w-full border border-gray-300 rounded px-3 py-2">
                        </div>
                        <div>
