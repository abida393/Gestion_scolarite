            ...
            <form action="{{ route('responsable.update', $emploiTemps->id) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $emploiTemps->classe->nom_classe }}"
                    ...
                    <div>
                        ...
                        <select name="jour" id="jour"
                            ...
                            @foreach (['Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi'] as $jour)
                                <option value="{{ $jour }}" {{ $emploiTemps->jour == $jour ? 'selected' : '' }}>
                                    {{ $jour }}</option>
                            @endforeach
                        </select>
                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="date" id="date"
                            value="{{ \Carbon\Carbon::parse($emploiTemps->date)->format('Y-m-d') }}"
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                            required>

                        <!-- Heure début -->
                        <div>
                            <label for="heure_debut" class="block text-sm font-medium text-gray-700 mb-1">Heure
                                début</label>
                            <input type="time" name="heure_debut" id="heure_debut"
                                value="{{ $emploiTemps->heure_debut }}"
                                class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                required>
                        </div>
                        ...
                        <!-- Heure fin -->
                        <div>
                            <label for="heure_fin" class="block text-sm font-medium text-gray-700 mb-1">Heure
                                fin</label>
                            <input type="time" name="heure_fin" id="heure_fin" value="{{ $emploiTemps->heure_fin }}"
                                class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                required>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <select name="matiere_id" id="matiere_id"
                                ...
                                @foreach ($matieres as $matiere)
                                    <option value="{{ $matiere->id }}"
                                        {{ $emploiTemps->matiere_id == $matiere->id ? 'selected' : '' }}>
                                        {{ $matiere->nom_matiere }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label for="enseignant_id"
                                class="block text-sm font-medium text-gray-700 mb-1">Enseignant</label>
                            <select name="enseignant_id" id="enseignant_id"
                                ...
                                @foreach ($enseignants as $enseignant)
                                    <option value="{{ $enseignant->id }}"
                                        {{ $emploiTemps->enseignant_id == $enseignant->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="salle" id="salle" value="{{ $emploiTemps->salle }}"
                            class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                            required>
                    </div>
