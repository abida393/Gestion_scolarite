<x-admin titre="paiement" page_titre="paiement" :nom_complete="Auth::guard('responsable')->user()->respo_nom . ',' . Auth::guard('responsable')->user()->respo_prenom">
    <div class="container mx-auto px-4 py-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="etudiant_id"
                                class="block text-sm font-medium text-gray-700 mb-1">Étudiant</label>
                            <select id="etudiant_id" name="etudiant_id"
                                class="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                required onchange="updateMontantTotal()">
                                <option value="">-- Sélectionner un étudiant --</option>
                                @foreach ($etudiants as $etudiant)
                                <option value="{{ $etudiant->id }}"
                                    data-total = "{{ $etudiant->formation->prix ?? 0 }}"
                                    data-montant="{{ $etudiant->paiements->last()->montant_restant ?? $etudiant->formation->prix }}"
                                    data-last-payment="{{ $etudiant->paiements->last()->montant_paye ??0}}">
                        ...
                        <!-- Total Amount -->
                        <div>
                            <label for="montant_total" class="block text-sm font-medium text-gray-700 mb-1">Montant
                                total</label>
                            <input type="number" step="0.01" name="montant_total" id="montant_total"
                                class="w-full p-2 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
                                readonly required>
                        </div>

                        <!-- Paid Amount -->
                        <div>
                            <label for="montant_paye" class="block text-sm font-medium text-gray-700 mb-1">Montant
                                payé</label>
                            <input type="number" step="0.01" name="montant_paye" id="montant_paye"
                                class="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                required oninput="calculateMontantRestant()">
                        </div>

                        <!-- Remaining Amount -->
                        <div>
                            <label for="montant_restant" class="block text-sm font-medium text-gray-700 mb-1">Montant
                                restant</label>
                            <input type="number" step="0.01" name="montant_restant" id="montant_restant"
                                class="w-full p-2 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500"
                                readonly required>
                        </div>
                        ...
                        <!-- Payment Method -->
                        <div>
                            <label for="mode_paiement" class="block text-sm font-medium text-gray-700 mb-1">Mode de
                                paiement</label>
                            <select name="mode_paiement" id="mode_paiement"
                                class="w-full p-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500"
                                required onchange="toggleChequeInput()">
                        ...
                        <div id="cheque_section" class="hidden">
                            <label for="numero_cheque" class="block text-sm font-medium text-gray-700 mb-1">Numéro de
                                chèque</label>
                        ...
                        <!-- Payment Date -->
                        <div>
                            <label for="date_paiement" class="block text-sm font-medium text-gray-700 mb-1">Date de
                                paiement</label>
        ...
        <form method="GET" action="{{ route('paiements.index') }}" class="mb-4">
            <input type="text" name="search" value="{{ request('search') }}"
                placeholder="Rechercher par nom ou prénom..." class="border p-2 rounded w-1/3" />
                            ...
                            @forelse ($paiements as $paiement)
                                <tr>
                                    <td class="px-3 py-4 whitespace-nowrap text-sm text-gray-900">
                                        {{ $paiement->etudiant->etudiant_nom }}
                                        {{ $paiement->etudiant->etudiant_prenom }}
                                    </td>
                ...
                @else bg-yellow-100 text-yellow-800 @endif">
                                        ...
                                        @if ($paiement->mode_paiement === 'cheque' && $paiement->status === 'en attente')
                                            <div class="mt-1 flex flex-col sm:flex-row gap-1">
                                                <form action="{{ route('paiements.changer-statut', $paiement->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <input type="hidden" name="status" value="confirmé">
                                                    <button type="submit"
                                                        class="text-xs text-green-600 hover:text-green-800 hover:underline">Confirmer</button>
                                                </form>
                                                ...
                                                <form action="{{ route('paiements.changer-statut', $paiement->id) }}"
                                                    ...
                                                    <button type="submit"
                                                        class="text-xs text-red-600 hover:text-red-800 hover:underline">Refuser</button>
                                                </form>
                                            </div>
                                        @endif
                                    </td>
    ...
    <script>
        function updateMontantTotal() {
            ...
            // Get all required values from data attributes
            const totalAmount = parseFloat(selectedOption.getAttribute('data-total')) || 0;
            const lastPayment = parseFloat(selectedOption.getAttribute('data-last-payment')) || 0;
            const remainingAmount = parseFloat(selectedOption.getAttribute('data-montant')) || totalAmount;

            // Set the form values
            document.getElementById('montant_total').value = totalAmount;
            document.getElementById('montant_paye').value = lastPayment;
            document.getElementById('montant_restant').value = remainingAmount.toFixed(2);
        }

        function calculateMontantRestant() {
            const select = document.getElementById('etudiant_id');
            const selectedOption = select.options[select.selectedIndex];
            const currentRemaining = parseFloat(selectedOption.getAttribute('data-montant')) || 0;
            const paye = parseFloat(document.getElementById('montant_paye').value) || 0;

            // Calculate new remaining amount
            const restant = currentRemaining - paye;
        ...
        function toggleChequeInput() {
            ...
            chequeSection.classList.toggle('hidden',
