...
<div id="deleteEventModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 opacity-0 invisible transition-all duration-300">
...
<!-- MODAL D'ÉDITION -->
<div id="editEventModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 opacity-0 invisible transition-all duration-300">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md transform transition-all duration-300 scale-95">
        <!-- HEADER BLEU -->
        <div class="bg-gradient-to-r from-blue-500 to-blue-700 p-6 rounded-t-xl flex justify-between items-center">
            ...
            <button onclick="closeEditModal()" class="text-white hover:text-gray-200 text-2xl font-bold leading-none">
                &times;
            </button>
        </div>
        <!-- FORMULAIRE -->
        <form id="editEventForm" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="title" id="edit_title" class="w-full border border-gray-300 rounded-lg p-3" required>
            </div>
            <div class="flex items-center">
                <input type="checkbox" name="is_fixed" id="edit_is_fixed" value="1" class="h-5 w-5 text-blue-600 rounded border-gray-300">
                <label for="edit_is_fixed" class="ml-2 text-gray-700">Événement fixe (récurrent)</label>
            </div>
            <div class="flex justify-end space-x-4">
                <button type="button" onclick="closeEditModal()" class="px-6 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Annuler</button>
                <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg">Enregistrer</button>
            </div>
        </form>
    </div>
</div>
...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        ...
        <button onclick="openEventForm()" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center space-x-2 transition-all duration-300">
    ...
    <div id="eventFormModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 opacity-0 invisible transition-all duration-300">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg transform transition-all duration-300 scale-95">
            <!-- EN-TÊTE MODAL -->
            <div class="bg-gradient-to-r from-blue-600 to-blue-800 p-6 rounded-t-xl flex justify-between items-center">
            ...
            <!-- CORPS DU FORMULAIRE -->
            <form method="POST" action="{{ route('responsable.calendar.plan.store') }}" id="eventForm" class="p-6 space-y-6">
                @csrf
                ...
                <div id="newEventSection" class="hidden space-y-4">
                    ...
                    <div class="flex items-center space-x-3">
                        ...
                        <label for="new_event_fixed" class="text-sm font-medium text-gray-700">Événement fixe (récurrent)</label>
                    </div>
                </div>
                <!-- DATES -->
                <div class="grid grid-cols-2 gap-4">
...
<style>
    ...
    .fc-header-toolbar { @apply mb-4; }
    .fc-button { @apply bg-white text-gray-800 border border-gray-300 hover:bg-gray-100 transition-colors duration-200; }
    .fc-button-primary { @apply bg-blue-600 text-white border-blue-600 hover:bg-blue-700 hover:border-blue-700; }
    .fc-daygrid-day-number, .fc-col-header-cell-cushion { @apply text-gray-700 hover:text-blue-600; }
    .fc-day-today { @apply bg-blue-50; }
    .fc-event { @apply bg-blue-600 border-blue-600 text-white rounded-lg px-2 py-1 text-sm cursor-pointer; }
    .fc-event:hover { @apply bg-blue-700 border-blue-700; }
</style>
<script>
